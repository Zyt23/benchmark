...
def data_provider(args, flag):
    ...
    if args.task_name == 'anomaly_detection':
        ...
        # QAR 数据已在 __getitem__ 内固定为 (seq_len, C) 并自带 mask，
        # 使用默认 default_collate 直接 stack；其余沿用 UEA 的变长 collate_fn。
        if args.data in ('QAR', 'QAR_shift'):
            data_loader = DataLoader(
                data_set,
                batch_size=batch_size,
                shuffle=shuffle_flag,
