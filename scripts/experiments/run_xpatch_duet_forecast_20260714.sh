...
for anchor in ${FORECAST_ANCHORS}; do
  ...
  log="${LOG_ROOT}/fullshot_forecast_${anchor}_xpatch_duet.log"
  pid_file="${LOG_ROOT}/fullshot_forecast_${anchor}_xpatch_duet.pid"
  echo "[launch] ${anchor} gpu=${gpu} -> ${log}"
  (
    env \
      RUN_TAG="matrix_fullshot_${anchor}_xpatch_duet_20260714" \
