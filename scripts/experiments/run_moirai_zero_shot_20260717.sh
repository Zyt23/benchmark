...
RUN_TAG_SUFFIX="${RUN_TAG_SUFFIX:-moirai}"
...
for anchor in ${FORECAST_ANCHORS}; do
  ...
  (
    env \
      RUN_TAG="matrix_zeroshot_${anchor}_${RUN_TAG_SUFFIX}_20260717" \
      ...
      LABEL_LEN=20 \
      PRED_LEN=20 \
      MOIRAI_MODEL_PATH="${MOIRAI_MODEL_PATH}" \
      HF_HUB_OFFLINE=1 \
...
echo "[done] launched Moirai zero-shot forecast jobs"
